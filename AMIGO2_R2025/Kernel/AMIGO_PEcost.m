        ...
        for iexp=1:inputs.exps.n_exp
            ...
            switch inputs.PEsol.PEcost_type
                ...
                case 'lsq'
                    ...
                    resM = ms(inputs.exps.nanfilter{iexp})-inputs.exps.exp_data{iexp}(inputs.exps.nanfilter{iexp});
                    
                               switch inputs.PEsol.lsq_type
                        
                        case 'Q_mat'
                            % weighting matrix:
                            QQ=inputs.PEsol.lsq_Qmat{iexp}(inputs.exps.nanfilter{iexp});
                            Q = reshape(QQ,nexpdata(iexp),1);
                            scaledResM = resM.*Q;      
                        ...
                        case 'Q_expmax'
                            ...
                            % weighting factor:
                            QQ = repmat(1./expDataMax, inputs.exps.n_s{iexp},1);
                            
                            Q=QQ(inputs.exps.nanfilter{iexp});
                            ...
                            case 'Q_expmaxmin'
                            expDataMax = max(inputs.exps.exp_data{iexp});
                            expDataMin= min(inputs.exps.exp_data{iexp});
                            % every timepoint of each observable have the same
                            % weighting factor:
                            QQ = repmat(1./(expDataMax-expDataMin), inputs.exps.n_s{iexp},1);
                            Q=QQ(inputs.exps.nanfilter{iexp});
                            % handles close infinity weights / clsoe zero experiments:
                            tmp1 = or(isnan(Q),isinf(Q));
                            Q(tmp1) = 1;
                        ...
                        case 'Q_expmean'
                            ...
                            QQ = repmat(1./expDataMean, inputs.exps.n_s{iexp},1);
                            Q=QQ(inputs.exps.nanfilter{iexp});
